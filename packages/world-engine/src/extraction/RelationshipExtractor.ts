...
export class RelationshipExtractor {
  private readonly _rules: RelationshipRule[] = [];

  public registerRule(rule: RelationshipRule): this {
   ...
   */
  public extractRelationships(entities: Entity[]): Relationship[] {
        ...
        if (rule.targetIdResolver) {
          ...
          if (entityMap.has(directTarget)) {
            ...
            for (const [id, e] of entityMap) {
              if (
                (rule.targetEntityType === '*' || e.type === rule.targetEntityType) &&
    ...
    for (const entity of entities) {
      for (const [propKey, propVal] of Object.entries(entity.properties)) {
        if (typeof propVal !== 'string' && typeof propVal !== 'number') continue;
        ...
        if (propKey.endsWith('Id') || propKey.endsWith('_id') || propKey.endsWith('Ref')) {
          if (entityMap.has(strVal) && strVal !== entity.id) {
            ...
            if (!relationType || relationType === 'PARENT') relationType = 'DEPENDS_ON';
            else relationType = `RELATES_TO_${relationType}`;
   ...
   */
  public extractFromInteractions(
    interactions: Array<{ callerId: string; calleeId: string; callCount?: number; avgLatencyMs?: number }>
  ): Relationship[] {
    const relationships: Relationship[] = [];
    ...
    for (const item of interactions) {
      const rel = new Relationship({
        ...
        attributes: {
          avgLatencyMs: (item.avgLatencyMs ?? 0) as PropertyValue,
          callCount: (item.callCount ?? 1) as PropertyValue,
        },
        metadata: {
