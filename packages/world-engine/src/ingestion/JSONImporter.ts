...
export interface JSONImportOptions {
  ...
  readonly nestedEntityTypes?: Record<string, string>; // propertyKey -> targetEntityType
}
...
export interface JSONImportResult {
  ...
  readonly parseErrors: Array<{ index: number; error: string }>;
}
...
export class JSONImporter {
   ...
   */
  public static importString(jsonText: string, options: JSONImportOptions = {}): JSONImportResult {
    ...
    for (let i = 0; i < rawItems.length; i++) {
      ...
      const flatProperties: Record<string, PropertyValue> = {};

      for (const [key, value] of Object.entries(record)) {
        if (
          options.extractNestedEntities &&
          typeof value === 'object' &&
          value !== null &&
          !Array.isArray(value)
        ) {
          ...
          const childEntity = new Entity({
            ...
            type: childType,
            name: childName,
            state: childObj as Record<string, PropertyValue>,
            metadata: {
          ...
          for (let j = 0; j < value.length; j++) {
            const childObj = value[j] as Record<string, unknown>;
          ...
          flatProperties[key] = value as PropertyValue;
        }
      }
